...
def test_hasCapitalLetter():
	out = main_testing.hasCapitalLetter("Test")
	out1 = main_testing.hasCapitalLetter("test")
	assert out == True
	assert out1 == False

# test hasDigit Function
def test_hasDigit():
	out = main_testing.hasDigit("Test123")
	out1 = main_testing.hasDigit("test")
	assert out == True
	assert out1 == False
...
def test_hasSpecialCharacter():
	...
	for char in specialCharacters:
		password = "test" + char
		out = main_testing.hasSpecialCharacter(password)
		assert out == True
	out = main_testing.hasSpecialCharacter("test")
	assert out == False

def test_usernameCreation(capfd, monkeypatch, database):
	# this will test is a unique username is inputted
	monkeypatch.setattr('builtins.input', lambda _: "test")
	result = main_testing.usernameCreation()
	assert result == "test"
...
def test_passwordCreation(capfd, monkeypatch):
	...
	result = main_testing.passwordCreation()
	out, err = capfd.readouterr()
...
def test_createAccount(monkeypatch, database):
	...
	result = main_testing.createAccount()
...
def test_userVerification():
	...
	result = main_testing.userVerification("Test", "Testing!123")
	assert result == True

	# test case for incorrect username
	result = main_testing.userVerification("no_username", "Testing!123")
	assert result == False

	# test case for incorrect password
	result = main_testing.userVerification("Test", "testing324")
	assert result == False
